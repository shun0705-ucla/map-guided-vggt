...
from train_utils.depthmap_process import make_sparse_depths_torch, make_normals_torch
...
class Trainer:
    ...
    def _setup_components(self):
        ...
        self.model = instantiate(self.model_conf, _recursive_=True)
    ...
    def _step(self, batch, model: nn.Module, phase: str, loss_meters: dict):
        ...
        sparse_depths, sparse_mask = _make_sparse_depths_torch(
            depths=batch["depths"],
            ratio=0.1,
            valid_mask=batch["point_masks"],   # if not provided, depths>0
            return_mask=True,
        )
        '''
        normals = make_normals_torch(
            depths=batch["depths"],
            K=batch["intrinsics"],
            valid_mask=batch["point_masks"],
            )
        #y_hat = model(x=batch["images"],d=sparse_depths)
        y_hat = model(images=batch["images"], aux=normals)
