...
class FusionTransformer(nn.Module):
    ...
    ########## for step-by-step layer execution ###########
    ...
    def init_layer_state(self, x: torch.Tensor, **kwargs) -> dict:
        # x: (B,S,C,H,W)
        B, S, _, H, W = x.shape

        x = self.prepare_tokens(x)  # (B,S,N,C)
        pos, pos_nodiff = self._prepare_rope(B, S, H, W, x.device)
    ...
    def process_one_layer(self, state: dict, **kwargs):
        i = state["i"]
        if i >= len(self.blocks):
            raise StopIteration("All layers processed")

        B, S = state["B"], state["S"]
        x = state["x"]
        local_x = state["local_x"]
        pos, pos_nodiff = state["pos"], state["pos_nodiff"]
        ...
        if state["did_reorder"] and state["b_idx"] is not None:
            out_x = restore_original_order(out_x, state["b_idx"])
